...
class Q_Learning(BaseAlgorithm):
    def __init__(
        ...
        super().__init__(
            ...
            action_range=action_range,  # [min, max]
            discretize_state_weight=discretize_state_weight, # [pose_cart:int, pose_pole:int, vel_cart:int, vel_pole:int]
    ...
    def update(
        ...
        current_q_value = self.q_values[obs_dis][action_idx]
        term = reward + (self.discount_factor * np.amax(self.q_values[next_obs_dis])) - current_q_value
        new_q_value = current_q_value + (self.lr * term)
        
        self.q_values[obs_dis][action_idx] = new_q_value
